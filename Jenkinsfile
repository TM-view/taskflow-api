    ...
    stages {
        ...
        stage('11. Container Scan (Trivy)') {
            steps {
                ...
                sh "npx trivy image --format sarif -o trivy.sarif ${REGISTRY}/${APP_NAME}:${IMAGE_TAG} || true"
                sh "npx trivy image --exit-code 1 --severity HIGH,CRITICAL ${REGISTRY}/${APP_NAME}:${IMAGE_TAG}"
            }
            post {
                always {
